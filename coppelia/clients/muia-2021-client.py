...
def avoid(sonar):
    L=1; FL=3; FR=4; R=6
    # Se va a chocar de frente
    if (sonar[FL] < 0.3) and (sonar[FR] < 0.3):
        lspeed, rspeed = (+0.5, -0.5) if sonar[FL] < sonar[FR] else (-0.5, +0.5)
    # Está muy cerca por la izquierda
    elif sonar[L] < 0.3:
        lspeed, rspeed = +0.7, +0.2
    # Está muy cerca por la derecha
    elif sonar[R] < 0.3:
        lspeed, rspeed = +0.2, +0.7
    # No se va a chocar con nada
    else:
